...
public class NotaXMLService extends AbstractXMLService<Integer,Nota>{
    ...
    protected Nota extractEntity(String[] params){
        ...
        try{
            ...
            ldt = LocalDateTime.parse(params[4] + " 00:00", formatter);
        } catch (DateTimeParseException e){
            System.out.println(e.getMessage());
        }
        Integer idNota = null;
        Integer temaLabId = null;
        double valoare = 0;
        try {
            idNota = Integer.parseInt(params[0]);
            temaLabId = Integer.parseInt(params[2]);
            valoare = Double.parseDouble(params[3]);
        } catch (NumberFormatException e){
            System.out.println(e.getMessage());
        }
        Nota n=new Nota(idNota,params[1],temaLabId,valoare, ldt);
        return n;
